...
- 修复 Biome 明确报错。
- 删除 `apps/user-web/package-lock.json`，避免 pnpm workspace 与 npm lock 混用。
...
- 停止旧 dev server 并清理 `apps/user-web/src/.umi`、`apps/user-web/node_modules/.cache/mfsu*` 后重新启动，首页和内容页不再出现该开发态 overlay。

后续如果调整 Umi、Vite、Vitest、Webpack、React-PDF 等构建期依赖，建议执行一次干净重启：

```bash
rm -rf apps/user-web/src/.umi apps/user-web/src/.umi-production apps/user-web/node_modules/.cache/mfsu apps/user-web/node_modules/.cache/mfsu-deps
corepack pnpm --filter user-web dev
```
...
1. `corepack pnpm --filter user-web test`
2. `corepack pnpm --filter user-web biome:lint`
3. `corepack pnpm typecheck`
4. `corepack pnpm --filter user-web tsc`
