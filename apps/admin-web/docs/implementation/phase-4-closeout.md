...
后台 mock 数据集中在 `apps/user-web/mock/data/admin-store.ts`：
...
`apps/user-web/mock/admin.ts` 提供对应 mock API：
...
内容状态和工作区列表共享 `apps/user-web/mock/data/contents.ts`，后台发布、归档、删除后，工作区与后台刷新都能看到同一份 mock 状态。
...
- `apps/user-web/src/services/admin/index.ts` 补齐后台 service 契约，页面层只依赖 service，不直接读 mock。
