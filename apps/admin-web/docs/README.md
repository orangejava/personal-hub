...
本地 `pnpm dev:admin` 监听 `:8001`，资源前缀 `/`。生产若由 Nginx 把 `/admin` 指到本应用静态资源，构建时设置 `PUBLIC_PATH=/admin/`。生产进程模型见仓库 `docs/deploy/nest-compose-strategy.md`（Nginx + Compose，不要用 PM2）。
