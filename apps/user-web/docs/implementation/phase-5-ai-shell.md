...
- `apps/user-web/mock/data/ai-store.ts`
- `apps/user-web/mock/ai.ts`
- `apps/user-web/mock/data/admin-store.ts`
- `apps/user-web/mock/admin.ts`
...
- `apps/user-web/src/services/ai.ts`
- `apps/user-web/src/services/ai.ts` 中新增 `generateAiText`、`generateAiImage`、`generateAiVideo`，用于文本、图片、视频生成 mock 契约。
- `apps/user-web/src/services/admin/index.ts` 中新增 `fetchAdminAiConfig`、`fetchAdminAiStats`。
- `apps/user-web/src/services/admin/index.ts` 中新增 `updateAdminAiBrandingConfig`，用于后台 AI 品牌配置 mock 保存。
- `apps/user-web/src/services/admin/index.ts` 中新增 `updateAdminAiProviderConfig`，用于后台 AI 厂商基础配置 mock 保存。
- `apps/user-web/src/services/admin/index.ts` 中新增 `updateAdminAiToolStatus`，用于后台 AI 工具状态 mock 保存。
- `apps/user-web/src/services/admin/index.ts` 中新增 `updateAdminAiToolConfig`，用于后台 AI 工具完整配置 mock 保存。
- `apps/user-web/src/services/admin/index.ts` 中新增 `moveAdminAiToolSort`，用于后台 AI 工具上移/下移 mock 保存。
- `apps/user-web/src/services/admin/index.ts` 中新增 `updateAdminAiModelConfig`，用于后台 AI 模型基础配置 mock 保存。
- `apps/user-web/src/services/admin/index.ts` 中新增 `createAdminAiModelConfig`、`deleteAdminAiModelConfig`，用于后台 AI 模型新增/删除 mock 保存。
- `apps/user-web/src/services/workspace.ts` 中新增 `fetchWorkspaceAiHistory`、`renameWorkspaceAiHistory`、`deleteWorkspaceAiHistory`、`batchDeleteWorkspaceAiHistory`。
- `apps/user-web/src/services/ai.ts` 中新增 AI 资产移入回收站、恢复、永久删除和批量操作接口。
- `apps/user-web/src/services/ai.ts` 中新增 `createAiAsset`，用于保存 AI 生成结果到资产库。
- `apps/user-web/src/services/ai.ts` 中新增 `consumeAiQuota`，用于生成成功后的 mock Token 扣减和余额刷新。
- `apps/user-web/src/services/ai.ts` 中新增 `moveAiAssetToFolder`、`batchMoveAiAssetsToFolder`，用于单个和批量移动资产到项目文件夹。
- `apps/user-web/src/services/ai.ts` 中新增 `fetchAiAssetFolders`、`createAiAssetFolder`、`renameAiAssetFolder`、`deleteAiAssetFolder`，用于阶段 5 项目文件夹 mock 管理。
- `apps/user-web/src/services/ai.ts` 中新增 `createAiSession`、`updateAiSession`、`deleteAiSession`、`persistAiChatMessages`，用于 Chat 会话、会话级设置和完成消息写入统一 mock store。
- `apps/user-web/src/services/ai.ts` 中新增 `updateAiMessageFeedback`，用于保存或取消 AI 回复点踩反馈。
...
- `apps/user-web/src/layouts/AiLayout/index.tsx`
...
- `apps/user-web/src/components/ai-x/`
...
- `apps/user-web/src/pages/Welcome.tsx` 已改为使用 `AiXMarkdown`，消除页面层直接依赖 Ant Design X Markdown 的例外。

AI 业务组件：

- `apps/user-web/src/components/ai/AiAttachmentPicker`
- `apps/user-web/src/components/ai/AiGuestLimitAlert`
- `apps/user-web/src/components/ai/AiPageHeader`
- `apps/user-web/src/components/ai/AiTemplateCard`
- `apps/user-web/src/components/ai/AiGenerationStream`
- `apps/user-web/src/components/ai/AiQuotaAlert`

AI 占位工具页：

- `apps/user-web/src/pages/ai/PlaceholderTool/index.tsx`
...
- `apps/user-web/src/pages/ai/Home/index.tsx`
  - 展示剩余额度、可用工具、最近创作统计。
...
- `apps/user-web/src/components/ai/AiReferencePicker`
- `apps/user-web/src/components/ai/hooks/useAiAvailableModels`
- `apps/user-web/src/components/ai/hooks/useAiChatMock`
- `apps/user-web/src/components/ai/hooks/useAiGenerationMock`
...
- `apps/user-web/src/pages/ai/Chat/index.tsx`
...
- `apps/user-web/src/pages/ai/Video/index.tsx`
  ...
  - 生成并保存资产成功后扣减预计 Token，顶部余额随 `runtimeConfig.quota` 更新。
  - 未登录时展示访客试用提示，`guestLimit=exceeded` 时禁用生成按钮并引导登录。
- `apps/user-web/src/pages/ai/Text/index.tsx`
...
- `apps/user-web/src/pages/admin/AiConfig/index.tsx`
...
- `apps/user-web/src/pages/admin/AiStats/index.tsx`
  - 展示消耗 Token、调用次数、预估费用、活跃用户。
  - 展示近 7 天趋势、工具消耗占比、用户消耗排行和模型消耗分布。

工作区 AI 历史：

- `apps/user-web/src/pages/workspace/AiHistory/index.tsx`
...
- `apps/user-web/src/pages/public/ContentDetail/index.tsx`
  - 在详情操作区新增“引用到 AI”按钮。
...
- `apps/user-web/src/pages/ai/Assets/index.tsx`
...
- `apps/user-web/src/styles/ai-tokens.less`
- `apps/user-web/src/styles/ai-layout.less`
- `apps/user-web/src/styles/ai-components.less`
- `apps/user-web/src/styles/ai-motion.less`
...
- `PATH=/Users/wangchaocheng/.nvm/versions/node/v22.22.0/bin:$PATH corepack pnpm --filter user-web tsc`
- `PATH=/Users/wangchaocheng/.nvm/versions/node/v22.22.0/bin:$PATH corepack pnpm --filter user-web biome:lint`
- `rg "from ['\\\"]@ant-design/x|@ant-design/x-markdown|@ant-design/x-sdk" apps/user-web/src -n`：确认第三方 X 依赖只出现在 `src/components/ai-x` 封装层。
