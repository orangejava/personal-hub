# `apps/user-web` 前端应用设计

> 状态：✅ `apps/user-web` 当前实现说明；后续调整需以实际目录为准
> 最后更新：2026-08-29
...
`apps/user-web` 是用户端 React 应用（本地 `:8000`），用于快速完成当前产品文档中描述的主要功能：
...
后台管理台在独立应用 `apps/admin-web`（本地 `:8001`）。
...
- 同步脚本：`apps/user-web/scripts/sync-local-booklets.ts`
- 输出数据：`apps/user-web/mock/data/local-booklets.generated.ts`
