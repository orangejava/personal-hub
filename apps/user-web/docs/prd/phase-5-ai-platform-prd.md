...
> 目标：在 `apps/user-web` 中先用 mock 跑通 AI 平台完整前端体验，形成“独立 AI 工作台 + 工具创作流 + 用量/资产/配置闭环”，为后续 NestJS 或临时 Next API 接入保留稳定 service 契约。
...
| 阶段 | 当前判断 | 依据 | 阶段 5 影响 |
|---|---|---|---|
| 阶段 0：工程骨架 | 已完成 | `docs/completed/README.md`、`apps/user-web`、`packages/shared-types` 已存在 | 可直接在 React Web 内新增 AI 模块 |
...
- 当前 `/ai` 已有占位页：`apps/user-web/src/pages/ai/Home/index.tsx`。
- 当前已有 AI service 占位：`apps/user-web/src/services/ai.ts`。
- 当前已有 AI mock 占位：`apps/user-web/mock/ai.ts`。
- 后台菜单 mock 已预留 AI 工作台菜单：`apps/user-web/mock/data/admin-store.ts`。
