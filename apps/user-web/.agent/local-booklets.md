...
- 白名单：`apps/user-web/src/scripts/sync-allowlist.json`（非空则只同步列出的文件夹名）。
- 输出：`apps/user-web/mock/data/local-booklets.generated.ts`（gitignore）。
- 同步：`pnpm sync:booklets`（根目录）或 `pnpm --filter user-web sync:booklets`。
