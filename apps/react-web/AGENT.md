...
> 适用范围：`apps/react-web` 下所有代码。本文件是总规范，分模块细则见 `.agent/` 子目录。
...
- 包管理：根目录 pnpm workspace，包名 `react-web`
- Node：>=22（见根目录 `.nvmrc`）
...
- 三类布局：PublicLayout（`/`、`/content`、`/projects`、`/about`）、WorkspaceLayout（`/workspace/*`）、AdminLayout（`/admin/*`，本阶段仅占位）。
...
| 后台管理台 | `src/pages/admin` | `src/components/admin` | `.agent/admin.md` |
| AI 工具 | `src/pages/ai` | `src/components/ai` | 阶段 4 顶栏导航 + 占位页；阶段 5 完整功能 |
...
- 阶段完成前执行：`pnpm install`、`pnpm dev:react`、`pnpm --filter react-web lint`、`pnpm --filter react-web tsc`。
