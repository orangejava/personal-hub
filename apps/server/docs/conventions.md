...
> 权威范围：工程、命名、API 横切规则、数据访问、Redis、队列、日志与测试。领域端点见 [canonical-api.md](../../docs/backend/canonical-api.md)，数据实体见 [canonical-data-model.md](../../docs/backend/canonical-data-model.md)。
...
- 当前 HTTP 组合固定为 **Nest 11 + `@nestjs/platform-express`**；包版本与 middleware 组合以 [Nest 依赖目录](./nest-dependency-catalog.md) 为准。
...
- `apps/user-web` 的 `/api/workspace/*`、旧 `/api/contents/*` 等 mock 路径只作迁移线索，不是新 API 的规范。
