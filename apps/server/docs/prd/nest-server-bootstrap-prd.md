...
> 最后更新：2026-08-08
> 优先级：P0
> 关联：[实现记录](../implementation/foundation/nest-server-bootstrap.md)、[后端实现约定](../conventions.md)、[Canonical API](../../../docs/backend/canonical-api.md)、[Canonical 数据模型](../../../docs/backend/canonical-data-model.md)、[依赖目录](../nest-dependency-catalog.md)、[Compose 策略](../../../docs/deploy/nest-compose-strategy.md)
...
| 事项                                        | 唯一来源                                                    |
| ------------------------------------------- | ----------------------------------------------------------- |
| HTTP 前缀、响应、错误、分页与端点分区       | [Canonical API](../../../docs/backend/canonical-api.md)             |
| 表命名、UUID、Prisma 映射和数据约束         | [Canonical 数据模型](../../../docs/backend/canonical-data-model.md) |
| Nest 分层、Redis/队列、日志、测试和安全规则 | [后端实现约定](../conventions.md)                |
| 包选型与不可混用约束                        | [依赖目录](../nest-dependency-catalog.md)    |
| 本地/生产 Compose、存储和发布原则           | [Compose 策略](../../../docs/deploy/nest-compose-strategy.md)       |
...
```mermaid
flowchart LR
    React["apps/user-web Mock"] -.当前不对接.-> Server["apps/server Host Dev"]
