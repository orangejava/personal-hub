...
实施范围来自 [../../react-first/roadmap.md](../../react-first/roadmap.md)：
...
1. `pnpm dev:react` 可以启动 `apps/react-web`。
2. `apps/react-web` 沿用 Ant Design Pro 的整体视觉和布局能力。
...
- 修改 `apps/react-web/package.json` 的 `name` 为 `react-web`。
...
```bash
pnpm --filter react-web sync:booklets
```
...
pnpm --filter react-web typecheck
pnpm build:react
```
...
3. 在 `content-local/booklets/` 放入一本 Markdown 小册，执行 `pnpm --filter react-web sync:booklets`。
