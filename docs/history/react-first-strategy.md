...
> 仅供追溯，不作为当前开发依据。当前应用边界见 [React-first 入口](../react-first/README.md)。
> 最后更新：2026-06-27
...
当前阶段先在 `apps/user-web` 中用 React + Umi + Ant Design Pro 快速完成完整 Web 功能；后端长期仍使用 NestJS + PostgreSQL + Redis；后续将公开前台、内容阅读等需要 SEO 和服务端渲染的页面抽到 Next.js 15 中重写。
