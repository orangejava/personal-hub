...
> 状态：✅ React-first 当时的单应用架构（历史资料，不作为当前开发依据）
> 当前边界：用户端 `apps/user-web` + 管理端 `apps/admin-web`，见 [React-first 入口](../react-first/README.md)。
> 最后更新：2026-08-02
...
| 应用             | 阶段     | 职责                                                                        |
| ---------------- | -------- | --------------------------------------------------------------------------- |
| `apps/user-web` | 立即开始 | 用 React/Umi/Ant Design Pro 完成首版全量 Web 功能                           |
...
```mermaid
flowchart LR
    User["用户"] --> ReactWeb["apps/user-web<br/>React + Umi"]
...
```mermaid
flowchart LR
    User["用户"] --> ReactWeb["apps/user-web"]
...
如果 React 和 Next 同时需要 mock，可把 mock 数据从 `apps/user-web` 抽出来，避免两边重复。
...
- `apps/user-web` 能引用 `packages/shared-types`。
- mock 接口能返回统一响应结构。
