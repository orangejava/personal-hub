...
见 `docs/engineering/dev-credentials.md`。日常：`pnpm dev:server` + `pnpm dev:user` + `pnpm dev:admin`（默认无 mock），用 `owner@example.com` 登录后点「后台管理」。不要用 mock 测 Nest 内容或 AI。`pnpm dev:react` 仍指向用户端。
