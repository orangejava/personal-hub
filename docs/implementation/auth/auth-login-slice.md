...
5. 打开 `http://localhost:8000/user/login`，使用 [本地开发账号](../../engineering/dev-credentials.md)
...
- `apps/react-web/src/auth/session.ts`：内存 Access Token
- `apps/react-web/src/services/auth.ts`：refresh 仅在 401 清会话
- `apps/react-web/src/pages/user/login/index.tsx`：登录失败冷却读 `Retry-After`
- `apps/react-web/config/proxy.ts`：`/api/v1` → Nest `:3001`
