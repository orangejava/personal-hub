...
- `apps/react-web`：完整克隆 `ant-design/ant-design-pro` v6.0.2 后改造（删 `.git`、包名改为 `react-web`、接入 workspace、加 `@personal-hub/shared-types` 依赖与 Markdown 编辑/渲染依赖、加 `sync:booklets` 脚本）。
...
- 目录级规范：`apps/react-web/AGENT.md` + `.agent/` 分模块细则；`packages/shared-types/AGENT.md`。
...
- ✅ `apps/react-web` 能 import `@personal-hub/shared-types`（探针文件 `src/sharedTypesProbe.ts` tsc 无解析错误）。
