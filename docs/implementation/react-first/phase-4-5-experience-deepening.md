...
- `corepack pnpm --filter react-web test`
...
- 已通过 `corepack pnpm --filter react-web biome:lint`。
- 已通过 `corepack pnpm typecheck`。
- 已通过 `corepack pnpm --filter react-web test`。
- 已启动 `react-web` dev server，清理 MFSU 生成缓存后未再复现 core-js 空白页错误。
